---
name: consolidate-project-versions
description: Audit and govern versioned project folders using the project's own naming conventions; reconcile PRDs and unresolved issues; create and validate one complete next-version codebase; coordinate with `ai-project-delivery-orchestrator`; archive every predecessor after the new version is approved using the project's lifecycle structure; classify regenerable storage; and optionally prepare remote recovery. Use for multi-version requirement conflicts, AI requirement drift, incomplete latest code, repeated full project snapshots, numbered lifecycle archives, or local version-retention planning. Start read-only, require approval before project writes, never infer V1/V2/V3 as fixed, never upload automatically, and never delete or move content to Trash.
---
...
Read [references/governance-model.md](references/governance-model.md) before choosing names, resolving conflicts, creating a candidate, running project commands, archiving versions, carrying issues forward, or discussing remote recovery.
...
- Detect Git history, configured remotes, and authentication separately. Missing `gh` does not prove the user lacks Git or a GitHub account.
- When off-device recovery matters and no usable remote exists, explain the benefit once and offer user-chosen GitHub, GitLab, Gitea, NAS, external storage, or another destination. Accept a decline and continue local-only with the risk stated.
