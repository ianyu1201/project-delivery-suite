...
用户使用口语、语音转写或非技术表达时，不要求其先写专业 Prompt。先复述“我理解的对象、目标、当前状态、这一步动作、完成标准”，再把需求翻译为专业交付物。遇到“这个项目/当前文件夹/继续/开始前再加一个任务”等表达，按 [novice-intake-and-scope-control.md](novice-intake-and-scope-control.md) 锁定对象与范围。

需要详细分级、版本策略或救援流程时，完整读取 [lifecycle-and-scaling.md](lifecycle-and-scaling.md)。
...
不要用“文档很多”代替门禁，也不要用“AI 表示完成”代替证据。详细证据层级读取 [quality-and-evidence.md](quality-and-evidence.md)。

## 版本治理协作

完整读取 [version-governance-coordination.md](version-governance-coordination.md) 后再处理新版本目录、多版本冲突或历史归档。总控先固定 `workspace_root / active_version_root / archive_root / repo_root / staging_root`，再让版本治理返回下一版命名、候选目录和狭义状态。候选目录物化后，总控才能在该完整目录中启动开发和独立验收会话。
...
完整读取 [ui-implementation-and-runtime-acceptance.md](ui-implementation-and-runtime-acceptance.md)，并从 [UI_IMPLEMENTATION_AND_RUNTIME_ACCEPTANCE_CONTRACT.md](../assets/UI_IMPLEMENTATION_AND_RUNTIME_ACCEPTANCE_CONTRACT.md) 生成项目文件。只有用户确认可观察结果后，才能进入 S4。
...
当前 WorkBuddy 任务可以承担轻量总控，但要把状态写入文件。只在前一阶段交付物已冻结时规划下一阶段任务。WorkBuddy 5.3.11 当前验证基线没有可供本 Skill 调用的任务创建 API，因此默认生成完整、可复制的标题和启动包，标记 `task creation unsupported`，由用户在 UI 中手动新建任务；不得声称已直接创建或填写虚构 ID。

未来版本只有在 capability detection 证实存在语义明确的任务创建工具、且用户确认方案后才可直接调用。详细规则读取 [session-orchestration.md](session-orchestration.md)。
...
用户未指定项目根时，推荐目录只展示相对结构，并将根写为 `<待用户确认的项目根>`。不得默认生成用户主目录、时间戳目录、WorkBuddy 会话临时目录或发布者机器路径；只有用户确认平台当前工作区就是项目根后，才把该真实路径写入提案和下游启动包。
...
按 [artifact-and-folder-governance.md](artifact-and-folder-governance.md) 选择最小结构。可先预览：
...
首次建档后使用带结构化 frontmatter 的 `PROJECT_STATE.md` 模板，并运行 `validate_project_state.py --gate status`。S3 合同冻结并准备开发交接时必须通过 `--gate handoff`；未通过时继续修当前阶段，不生成可执行的开发任务启动包。
...
- PRD：优先 `prd-development` 或同类产品能力；缺失时不要阻断或假装已调用，改用 [MINIMUM_PRD.md](../assets/MINIMUM_PRD.md) 生成最小但可批准、可追溯的 PRD；
...
PRD 回退遵循以下顺序：先复用项目现有的权威 PRD 文件名、语言和结构；只有没有可用约定时才适配 `MINIMUM_PRD.md`。同一版本只维护一份现役 PRD，不另建“最终版”“汇总版”或平行基线。先展示草稿并处理开放问题，只有有权限的产品负责人批准精确内容并留下批准证据后才把状态改为 `approved`。PRD 批准不等于实现、验收或版本批准。
...
- `SESSION_REGISTRY.md`：会话、职责、分支和状态；
- 当前版本目录：范围、设计、合同、交付、验收和证据索引。

使用 [assets](../assets/) 中的模板，但先适配现场目录，不强行创建平行真相。已有权威文件时就地补齐或建立短指针。

每次更新当前版本、阶段或项目根后，同步检查 `PROJECT_STATE.md`、`AGENTS.md`、`README.md`、活动 PRD、UI 合同和 `SESSION_REGISTRY.md`。若项目被移动，旧绝对路径立即失去交接资格；先由用户确认新根并重绑定所有指针，再运行 status gate。
...
- 既有版本系列的增版和归档执行 [version-governance-coordination.md](version-governance-coordination.md)；验证失败时继续同一 `active_candidate`，不重复增版。

Git 与发布细则读取 [git-and-release-control.md](git-and-release-control.md)。
...
- [lifecycle-and-scaling.md](lifecycle-and-scaling.md)：入口、规模、阶段和版本策略。
- [session-orchestration.md](session-orchestration.md)：WorkBuddy 任务规划、手动启动包、命名、上下文和子智能体。
- [artifact-and-folder-governance.md](artifact-and-folder-governance.md)：目录、权威来源、素材和文档治理。
- [ui-implementation-and-runtime-acceptance.md](ui-implementation-and-runtime-acceptance.md)：UI 实现约束与运行验收合同。
- [quality-and-evidence.md](quality-and-evidence.md)：测试、运行验证、独立验收和证据。
- [git-and-release-control.md](git-and-release-control.md)：Git、worktree、提交、标签和版本。
- [project-profiles.md](project-profiles.md)：软件、iOS、Web、机器人和混合项目差异。
- [novice-intake-and-scope-control.md](novice-intake-and-scope-control.md)：口语需求还原、对象锁定、范围变化和绝对目标翻译。
- [version-governance-coordination.md](version-governance-coordination.md)：两个逻辑模块的路径、状态、候选和归档交接合同。
- [assets](../assets/)：建档、合同、交接和验收模板。
- [MINIMUM_PRD.md](../assets/MINIMUM_PRD.md)：没有专业 PRD Skill 或项目既有规范时的最小 PRD 回退模板。
