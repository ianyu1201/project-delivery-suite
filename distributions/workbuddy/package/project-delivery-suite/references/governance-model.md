...
The ordering is an invariant, not a conversational suggestion. An authorized user's request to approve or archive records authorization but cannot make a draft PRD, pending validation, pending acceptance, missing fixed point, or missing evidence current. Run the bundled state validator before handoff, version approval, and archival; a non-zero result prevents that transition.
...
In `complete-snapshot` topology, one complete current version remains active outside the archive and all predecessors live beneath the archive root. In `numbered-lifecycle` topology, materialize the complete candidate in isolated staging from the approved Git baseline, or use a worktree that the user has actually created and verified; WorkBuddy 5.3.11 does not promise automatic worktree creation. After approval, use the project's authorized Git integration policy to establish the single active code path in the engineering area. Keep current PRD/design material in their numbered lifecycle areas; move superseded version material beneath `90_历史归档/<version>`; and retain historical code through verified Git fixed points or explicitly approved complete snapshots. A branch or worktree alone is never approval or completeness evidence.
...
The closure matrix is eligible for completion only after `validate_project_state.py --gate status` passes and the last applicable transition gate has a retained passing result. If the project root moved, rebind all absolute pointers before any transition; stale paths make anti-drift failed rather than implicitly relocated.
