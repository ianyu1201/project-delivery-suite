...
组合包不依赖额外的 PRD Skill 才能启动。检测到 `prd-development` 或同类能力时会优先使用；未安装时自动采用内置最小 PRD 规范，仍能生成一份可追溯、可审批的权威 PRD。
...
产品方案确认后进入 PRD 阶段。专业 PRD Skill 属于可选增强能力；缺失时主控使用内置回退模板，覆盖目标用户、问题、范围与排除项、稳定需求 ID、状态与异常、验收条件、开放决策和批准证据。项目已有 PRD 时就地完善，不生成第二份平行文档。
